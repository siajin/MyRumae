...
| `folders.py` | `modtype_folder` 자료실의 첨부 다운로드 |
...
# 금지 — 인라인 셀렉터
await page.locator("li.section[data-sectionid]").all()
```
...
2. `app/collector/<modtype>.py` 생성 — `collect_<modtype>(page, *, db, course_id, course_name, cmid) -> int` 시그니처 유지
3. [course_page.collect_course_materials](course_page.py)의 dispatch에 `elif ref.modtype == "..."` 분기 추가
4. 필요하면 [db/models.py](../db/models.py)에 새 테이블 + [repository.py](../db/repository.py)에 upsert 함수
